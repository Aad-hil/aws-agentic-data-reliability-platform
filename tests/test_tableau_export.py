...
def test_transform_report_normalizes_character_by_character_agent_evidence() -> None:
    ...
    assert "The order_id column contains duplicates." in insights[0]["recommendation_rationale"]
    assert len(insights[0]["recommendation_rationale"]) < 500
...
def test_export_reports_writes_three_tableau_csvs(tmp_path: Path) -> None:
    ...
    reports_dir.mkdir()
    (reports_dir / "run-001.json").write_text(
